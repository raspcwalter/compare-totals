    ...
    try {
      ...
      if (fetchMethod === "allRecords") {
        ...
        if (contractData) {
          ...
          // You'll need to transform contractData to match your energyRecords format
          // setEnergyRecords(transformedData);
        } else {
          addLog("No contract data available yet")
        }
      } else {
        await fetchSingleRecord(clientData, year, month)
      }
    } catch (error) {
