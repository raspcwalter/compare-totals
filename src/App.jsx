...
function App() {
  ...
  // Modified to increment the refresh trigger when comparing tariffs
  const handleCompareTariff = () => {
    ...
    // If data is already loaded, we can refetch it
    if (data && !isLoading) {
      refetch()
    }
  }

  // Call Nebula API when data is loaded or refresh trigger changes
  useEffect(() => {
  ...
  }, [data, isLoading, error, showPrompt, refreshTrigger]) // Add refreshTrigger to dependencies
    ...
    <div className="max-w-4xl mx-auto p-6 bg-white shadow-lg rounded-lg">
      <div className="flex flex-col items-start">
        <div className="w-full flex justify-between items-center mb-6">
          <div>
            <img src={logo || "/placeholder.svg"} alt="logo" width="300 px" />
          </div>
          <ConnectButton
            client={client}
            chain={sepolia}
